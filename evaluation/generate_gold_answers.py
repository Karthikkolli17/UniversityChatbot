# Generates gold_answers.json from raw data files for RAG evaluation.
# For each test question: finds relevant chunks, synthesizes a gold answer via Groq,
# and writes {section, query, relevant_chunks, gold_answer} to gold_answers.json.
# Usage: python generate_gold_answers.py ["Calendar"]
...
def load_contacts():
    rows = []
    with open(DATA_DIR / "contacts_data.csv", newline="", encoding="utf-8") as f:
...
def load_documents():
    with open(DATA_DIR / "unstructured_chunks.json") as f:
        return json.load(f)
