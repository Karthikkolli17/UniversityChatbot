# Evaluates the chatbot across all test sections using six metrics.
# Calls search/retrieval functions directly — no frontend needed.
#
# Retrieval metrics (per query, top-k chunks):
#   Precision@k, Recall@k, HitRate@k
# Answer quality metrics (LLM-judged, 0.0–1.0):
#   Faithfulness, Completeness, Correctness
#
# Usage:
#   python evaluate_section_metrics.py --phase generate       # retrieve + generate all answers
#   python evaluate_section_metrics.py --phase judge          # judge saved answers (uses 70b)
#   python evaluate_section_metrics.py --phase judge --section Tuition
#   python evaluate_section_metrics.py                        # both phases
#   python evaluate_section_metrics.py --k 5
...
# ── Context builders (mirrors core/pipeline.py build_*_context functions) ────
...
# Calls Theta Cloud edge inference with retry+backoff for 409/5xx errors.
...
# ── Query rewriting (mirrors core/pipeline.py rewrite_query) ─────────────────

# Lightweight chat call for query rewriting — uses 8b-instant.
...
# Rewrites a natural-language question into retrieval-optimised language.
...
# Uses an LLM to choose the most appropriate clarification option.
# Falls back to the first option if the LLM response doesn't match.
...
# Auto-resolves a clarification dict by picking the best option and re-running
# the search with an augmented query. Returns (resolved_hits_list, was_clarified).
...
# Runs the full retrieval pipeline: route → rewrite → search (with auto clarification resolution).
# Returns (context_parts, domains, is_clarification).
...
# Generates an answer from retrieved context using 8b-instant (matches the live app model).
...
# LLM call for judging — uses Theta when USE_THETA is set, else Groq.
...
# Asks the LLM whether each of the top-k context chunks is relevant to the query.
# Returns a list of booleans of length min(k, len(chunks)).
...
# Single LLM call that returns faithfulness, completeness, and correctness as floats in [0, 1].
...
# Writes all per-question metrics and section/global summaries to JSON.
...
# Retrieves context and generates answers for all questions. Saves to generated_answers.json.
...
# Loads generated_answers.json, runs 70b judge for all 6 metrics, and writes the report.
