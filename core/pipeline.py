...
import utilities.slot_filling as _slot_mod

# Fallback used when slot_filling doesn't export contact_reply_matches_picker_option.
...
# ── LLM helpers (Groq primary, Azure OpenAI fallback) ────────────────────────

# Lightweight LLM call for intent classification and query reformulation.
# Tries Groq first; falls back to Azure OpenAI if Groq fails or is unconfigured.
...
# Full-length synthesis call using Azure OpenAI.
# Returns an empty string on failure so the caller can fall back to Groq.
...
# ── Clarification turn classifiers ───────────────────────────────────────────

# Returns True if the user is abandoning the clarification rather than answering it.
# Short filler-only replies are caught heuristically to avoid an unnecessary LLM call.
# Single student-level words (e.g. "graduate") are always treated as valid answers.
...
# Classifies the user's reply to a pending clarification as one of:
#   ANSWER  — the reply is responsive to the clarification question
#   NEW_TOPIC — the user switched to a different question entirely
#   CANCEL  — the user explicitly abandoned the pending question
# Falls back to is_escape() if Groq is unavailable.
def classify_pending_response(
    original_query: str,
    ...
    # Topic-switch signal word + wh-question → NEW_TOPIC without an LLM call.
    ...
    _WH_QUESTION = re.compile(
        r"\b(what|when|where|who|how|why|is there|are there|can I|do I)\b",
        re.IGNORECASE,
    )
    if _SWITCH_SIGNALS.match(response) and _WH_QUESTION.search(response):
        return "NEW_TOPIC"

    # A question whose words share fewer than 2 tokens with the option list is a topic switch.
    if response.rstrip().endswith("?") and clarification_options:
        ...
        if len(reply_words & opt_words) < 2:
            return "NEW_TOPIC"
...
# Combines the original question and the user's clarification into a single
# retrieval-ready question. Falls back to simple concatenation if the LLM fails.
...
# Returns True if the current query depends on the previous turn to be understood
# (e.g. uses a pronoun, ellipsis, or implicit reference to an earlier entity).
...
# Returns True if the query explicitly asks for information across multiple domains
# (contains "and", "also", or "plus"). Used to keep secondary clarifications alive.
def _looks_multi_part_query(query: str) -> bool:
    q = (query or "").strip().lower()
    return bool(q and re.search(r"\b(and|also|plus)\b", q))
...
def build_calendar_context(hits):
    ...
    for h in hits:
        ...
        # semantic_text is enriched with synonyms and plain-language descriptions;
        # fall back to event_name only when it's absent.
...
_SYSTEM_PROMPT = """You are Hawk, the official Illinois Institute of Technology virtual assistant.
...
# Returns a canned reply for greetings, meta-questions, and clearly off-topic inputs
# so they never reach the semantic router. Returns None if the query should be routed.
def _off_topic_short_reply(query: str) -> str | None:
    ...
    exact = {
        "how are you", "how are you?", "what's up", "whats up", "whats up?",
        "what's up?", "hey what's up", "hey whats up", "sup", "hey", "hi",
        "hello", "hi there", "hello there", "thanks", "thanks!", "thank you",
        "thank you!", "thx", "ty", "ok", "okay", "ok!", "cool", "nice",
        "never mind", "nevermind", "forget it", "no thanks", "no thank you",
        "it's broken", "its broken", "this is broken", "goodbye", "bye", "bye!",
        "goodbye!", "see you", "see ya", "good bye", "hello?", "hey there",
        "heyy", "heyyy", "can you help me", "can you help me?", "can you help",
        "help me", "help", "help!", "help me please", "please help",
        "please help me", "i need help", "i need some help", "i need assistance",
        "idk where to start", "i dont know where to start",
        "i have no idea what im doing", "i have no idea", "i dont know what to do",
        "i dont know", "i'm lost", "im lost", "this is so confusing",
        "this is confusing", "so confused", "i'm confused", "im confused",
        "nobody told me about any of this", "i dont know who to ask",
        "i don't know who to ask", "wait how does this work", "how does this work",
        "how does this work?", "umm", "um", "uh", "i have a question",
        "i have a question!", "i need info", "i need information",
    }
    if s in exact:
        return (
    ...
    if s in {"what?", "what", "huh", "huh?"}:
        return "I did not quite catch that — what do you need about the calendar, directory, tuition, or policies?"

    if s in {
        "who are you", "who are you?", "what are you", "what are you?",
        "what is this", "what is this?", "what can you do", "what can you do?",
        "what do you do", "what do you do?", "what even is this chatbot",
        "what is this chatbot", "what does this chatbot do", "what does this do",
        "what does this do?", "what even are you", "what even are you?",
    }:
        return (
    ...
    # Narrow pattern — only catches unambiguous academic-dishonesty requests.
...
# Returns the second-to-last user message from chat history (i.e. the previous turn),
# or an empty string if there are fewer than two user messages.
...
# Rewrites a colloquial user query into retrieval-optimized academic language.
# When context_hint is provided (previous turn), it is used for co-reference resolution
# (e.g. "what about part time?" → "part-time graduate tuition at Armour College").
...
# Builds the message list for the LLM synthesis call.
# Includes only the last exchange from history to prevent contamination from earlier turns.
...
# Runs synthesis: tries Azure OpenAI first, falls back to Groq.
# Returns an empty string if both fail (caller should handle gracefully).
def _synthesize(messages: list) -> str:
...
# Searches a specific domain directly, bypassing the router.
# Used for clarification follow-ups where the domain is already known.
# Returns the same 7-tuple as get_answer.
def get_answer_for_domain(
    query: str,
    domain: str,
    chat_history: list = None,
) -> tuple[str, list, dict, bool, str, str, list]:
    q = (query or "").strip()
    if not q:
    ...
    if total_hits == 0:
        if clarification_messages:
            ...
            return _format_clarification(raw_msg, options), [], route_details, True, raw_msg, domain, options
        return "I couldn't find anything matching that. Could you try rephrasing?", [], route_details, False, "", "", []

    context_text = "\n\n---\n\n".join(context_parts)
    unique_sources = list(dict.fromkeys(all_sources))
    messages = _build_messages(_SYSTEM_PROMPT, chat_history or [], q, context_text)
    answer = _synthesize(messages)

    if not answer:
        return f"**Context found:**\n\n{context_text}", unique_sources, route_details, False, "", "", []
    return answer, unique_sources, route_details, False, "", "", []

# Main entry point. Routes the query, retrieves context from relevant domains,
...
def get_answer(
    ...
    # The API path does not append the current question to chat_history before calling
    # get_answer, so _previous_user_utterance (which looks for users[-2]) returns "".
    # Fall back to the last user message in history instead.
    ...
    # Bypass the rewriter for tuition comparison queries — it has been observed to drift
    # into unrelated program descriptions (e.g. "compare tuition mies vs chicago-kent").
    if DOMAIN_TUITION in domains and re.search(r"\b(compare|comparison|vs|versus)\b", q.lower()):
        retrieval_query = q

    # Bypass the rewriter for standalone DOCUMENTS queries — adds noise that derails
    # retrieval (e.g. "international student visa policy" → vague paraphrase).
    if domains == [DOMAIN_DOCUMENTS] and not prev_user and len(q.split()) >= 2:
        retrieval_query = q

    # Detect whether the previous turn was a real answer (not a clarification or error)
    # to prevent is_followup from firing after unrelated turns in long sessions.
    ...
    if not last_was_answer and chat_history and len(chat_history) >= 2:
        ...
        if last_asst and not any(kw in last_asst.lower() for kw in [
            "could you specify", "which semester", "which school", "could you clarify", "options:",
        ]):
    ...
    all_sources = []
    total_hits = 0
    clarification_messages = []  # (domain, message, options)
    domain_hit_counts = {domain: 0 for domain in domains}
    primary_domain = domains[0] if domains else ""

    if DOMAIN_CALENDAR in domains:
        # Use the rewritten query so resolved follow-up context (e.g. "when do they end?" →
        # "when do spring 2026 classes end?") prevents spurious clarification prompts.
    ...
    if DOMAIN_DOCUMENTS in domains:
        ...
        try:
            ...
            if isinstance(hits, dict) and hits.get("needs_clarification"):
                ...
                hits = []
            # Also search with the original query — the rewriter can add noise that hurts
            # recall, so we merge both result sets and deduplicate by _id.
    ...
    # If the primary domain returned results, suppress secondary-domain clarifications
    # to avoid appending an unrelated prompt onto an otherwise grounded answer.
    if (
        primary_domain
    ...
    # Clarify if: (a) nothing was retrieved, or (b) the primary domain itself needs
    # clarification — we must not silently answer the wrong school/semester when
    # the user's intent is underspecified.
    if clarification_messages:
        ...
        if total_hits == 0 or primary_clarifications:
            clarifying_domain, raw_msg, options = (
                primary_clarifications[0] if primary_clarifications else clarification_messages[0]
            )
            return _format_clarification(raw_msg, options), [], route_details, True, raw_msg, clarifying_domain, options
