...
class _PatternCache:
    ...
    def tuition_specific(self):
        def _build():
            ...
            levels    = options_cache.tuition_levels    if _OPTIONS_AVAILABLE else []
            years     = options_cache.tuition_years     if _OPTIONS_AVAILABLE else []
            extra = ["upass", "u-pass", "deposit", "application fee"]
...
_CALENDAR_WEAK  = re.compile(
    r"\b(deadline|due|start|end|begin|close|week|weeks|first day|last day|"
    r"first class|last class|orientation)\b",
    re.IGNORECASE,
)
